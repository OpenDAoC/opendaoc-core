...
namespace DOL.GS.Friends
{
	...
	public sealed class FriendMgr
	{
		...
		public void AddPlayerFriendsListToCache(GamePlayer Player)
		{
			...
			if (friends.Any())
			{
				offlineFriends = Database.SelectObjects<DbCoreCharacters>(DB.Column("Name").IsIn(friends))
		...
		public bool AddFriendToPlayerList(GamePlayer Player, string Friend)
		{
			...
			if (success)
			{
				...
				var offlineFriend = Database.SelectObjects<DbCoreCharacters>(DB.Column("Name").IsEqualTo(Friend)).FirstOrDefault();
