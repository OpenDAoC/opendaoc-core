...
namespace DOL.GS.Spells
{
	...
	/// </summary>
	public class ArrowHandler : SpellHandler
	{
		/// <summary>
		...
		protected class ArrowOnTargetAction : RegionAction
		{
			...
			/// The spell handler
			/// </summary>
			protected readonly ArrowHandler m_handler;
			...
			public ArrowOnTargetAction(GameLiving actionSource, GameLiving arrowTarget, ArrowHandler spellHandler)
				: base(actionSource)
			{
				if (arrowTarget == null)
						...
						if (target.IsObjectInFront(caster, 180) && lefthand.Object_Type == (int)EObjectType.Shield)
						{
							...
							if (target.IsEngaging)
							{
								...
								if (engage != null && target.attackComponent.AttackState && engage.EngageTarget == caster)
								{
									...
									else if (engage.Owner.Endurance < EngageHandler.ENGAGE_DURATION_LOST)
									{
										engage.Cancel(false); // if player ran out of endurance cancel engage effect
									}
									else
									{
										engage.Owner.Endurance -= EngageHandler.ENGAGE_DURATION_LOST;
										if (engage.Owner is GamePlayer)
		...
		// constructor
		public ArrowHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell, line) { }
	}
}
