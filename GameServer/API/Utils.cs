...
public class Utils
{
    ...
    public IList<Player.PlayerInfo> GetTopRP()
    {
        ...
        if (!_cache.TryGetValue(_topRPKey, out IList<Player.PlayerInfo> topRP))
        {
            ...
            var topRpPlayers = CoreDb<DbCoreCharacters>.SelectObjects(DB.Column("RealmPoints").IsLessThan(7000000)).OrderByDescending(x => x.RealmPoints).Take(10).ToDictionary(x => x.Name, x => x.RealmPoints);
