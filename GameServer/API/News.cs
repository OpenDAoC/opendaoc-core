...
public class News
{
    ...
    public List<NewsInfo> GetAllNews()
    {
        ...
        if (cache == null)
        {
            ICollection<DbNews> newsList = CoreDb<DbNews>.SelectAllObjects().OrderByDescending(a => a.CreationDate)
    ...
    public List<NewsInfo> GetRealmNews(string realm)
    {
        ...
        if (cache == null)
        {
            ICollection<DbNews> newsList = CoreDb<DbNews>.SelectObjects(DB.Column("Realm").IsEqualTo(realm))
    ...
    public List<NewsInfo> GetTypeNews(string type)
    {
        ...
        if (cache == null)
        {
            ICollection<DbNews> newsList = CoreDb<DbNews>.SelectObjects(DB.Column("Type").IsEqualTo(type))
                .OrderByDescending(a => a.CreationDate).Take(_numNews).ToList();
