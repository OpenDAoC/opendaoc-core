	...
	public sealed class RelicMgr
	{
		...
		public static bool Init()
		{
			lock (m_relics.SyncRoot)
			{
				...
				foreach (GameStaticRelic rel in m_relics.Values)
				{
				...
				List<GameStaticRelic> lostRelics = new List<GameStaticRelic>();
				...
				foreach (DbRelics datarelic in relics)
				{
					...
					GameStaticRelic relic = new GameStaticRelic(datarelic);
				...
				foreach (GameStaticRelic lostRelic in lostRelics)
				{
		...
		public static int GetDaysSinceCapture(GameStaticRelic relic)
		{
		...
		private static GameRelicPad GetPadAtRelicLocation(GameStaticRelic relic)
		{
		...
		public static GameStaticRelic getRelic(int id)
		{
			return m_relics[id] as GameStaticRelic;
		}
		...
		public static IList getNFRelics()
		{
			...
			foreach (GameStaticRelic relic in m_relics.Values)
			{
				myRelics.Add(relic);
			}
			return myRelics;
		...
		public static IEnumerable getRelics(ERealm Realm, eRelicType RelicType)
		{
			...
			foreach (GameStaticRelic relic in getRelics(Realm))
			{
		...
		public static int GetRelicCount(ERealm realm)
		{
			...
			lock (m_relics.SyncRoot)
			{
				foreach (GameStaticRelic relic in m_relics.Values)
				{
					if ((relic.Realm == realm) && (relic is GameStaticRelic))
						index++;
				}
			}
			return index;
		...
		public static int GetRelicCount(ERealm realm, eRelicType type)
		{
			...
			lock (m_relics.SyncRoot)
			{
				foreach (GameStaticRelic relic in m_relics.Values)
				{
					if ((relic.Realm == realm) && (relic.RelicType == type) && (relic is GameStaticRelic))
						index++;
				}
			}
			return index;
		...
		public static double GetRelicBonusModifier(ERealm realm, eRelicType type)
		{
			...
			foreach (GameStaticRelic rel in getRelics(realm, type))
			{
		...
		/// <returns></returns>
		public static bool CanPickupRelicFromShrine(GamePlayer player, GameStaticRelic relic)
		{
			...
			foreach (GameStaticRelic curRelic in list)
			{
