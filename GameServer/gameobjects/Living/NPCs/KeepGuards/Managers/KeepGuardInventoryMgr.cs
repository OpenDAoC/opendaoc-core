...
namespace DOL.GS.Keeps
{
	...
	/// </summary>
	public class KeepGuardInventoryMgr
	{
		...
		{
			if(!ServerProperties.ServerProperties.AUTOEQUIP_GUARDS_LOADED_FROM_DB && !guard.LoadedFromScript)
			{
				return;
			}
            if (guard is FrontierHastener || guard is GateKeeperIn || guard is GateKeeperOut)
            {
                switch (guard.Realm)
                {
                    ...
                    case ERealm.Midgard:
                        {
                            guard.Inventory = KeepGuardInventoryMgr.Midgard_Hastener.CloneTemplate();
                            break;
                        }
                }
            }
				...
				case ERealm.Albion:
					{
						if (guard is GuardFighter)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Albion_FighterPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Albion_Fighter.CloneTemplate();
						}
						else if (guard is GuardCommander)
							guard.Inventory = KeepGuardInventoryMgr.Albion_Commander.CloneTemplate();
						else if (guard is GuardRelicKeepFighter)
                            guard.Inventory = KeepGuardInventoryMgr.Albion_FighterPK.CloneTemplate();
						else if (guard is KeepLord || guard is MissionMaster)
							guard.Inventory = KeepGuardInventoryMgr.Albion_Lord.CloneTemplate();
						else if (guard is GuardMerchant)
							guard.Inventory = KeepGuardInventoryMgr.Albion_Merchant.CloneTemplate();
						else if (guard is GuardCurrencyMerchant)
							guard.Inventory = KeepGuardInventoryMgr.Albion_Merchant.CloneTemplate();
						else if (guard is GuardHealer)
							guard.Inventory = KeepGuardInventoryMgr.Albion_Healer.CloneTemplate();
						else if (guard is GuardArcher)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Albion_ArcherPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Albion_Archer.CloneTemplate();
						}
						else if (guard is GuardCaster)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Albion_CasterPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Albion_Caster.CloneTemplate();
						}
						else if (guard is GuardStealther)
							guard.Inventory = KeepGuardInventoryMgr.Albion_Stealther.CloneTemplate();
						break;
					}
				case ERealm.Midgard:
					{
						...
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Midgard_FighterPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Midgard_Fighter.CloneTemplate();
						}
						else if (guard is GuardCommander)
							guard.Inventory = KeepGuardInventoryMgr.Midgard_Commander.CloneTemplate();
                        else if (guard is GuardRelicKeepFighter)
                            guard.Inventory = KeepGuardInventoryMgr.Midgard_FighterPK.CloneTemplate();
                        else if (guard is KeepLord|| guard is MissionMaster)
							guard.Inventory = KeepGuardInventoryMgr.Midgard_Lord.CloneTemplate();
						else if (guard is GuardMerchant)
							guard.Inventory = KeepGuardInventoryMgr.Midgard_Merchant.CloneTemplate();
						else if (guard is GuardCurrencyMerchant)
							guard.Inventory = KeepGuardInventoryMgr.Midgard_Merchant.CloneTemplate();
						else if (guard is GuardHealer)
							guard.Inventory = KeepGuardInventoryMgr.Midgard_Healer.CloneTemplate();
						else if (guard is GuardArcher)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Midgard_ArcherPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Midgard_Archer.CloneTemplate();
						}
						else if (guard is GuardCaster)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Midgard_CasterPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Midgard_Caster.CloneTemplate();
						}
						else if (guard is GuardStealther)
							guard.Inventory = KeepGuardInventoryMgr.Midgard_Stealther.CloneTemplate();
						break;
					}
				case ERealm.Hibernia:
					{
						...
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Hibernia_FighterPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Hibernia_Fighter.CloneTemplate();
						}
						else if (guard is GuardCommander)
							guard.Inventory = KeepGuardInventoryMgr.Hibernia_Commander.CloneTemplate();
                        else if (guard is GuardRelicKeepFighter)
                            guard.Inventory = KeepGuardInventoryMgr.Hibernia_FighterPK.CloneTemplate();
                        else if (guard is KeepLord || guard is MissionMaster)
							guard.Inventory = KeepGuardInventoryMgr.Hibernia_Lord.CloneTemplate();
						else if (guard is GuardMerchant)
							guard.Inventory = KeepGuardInventoryMgr.Hibernia_Merchant.CloneTemplate();
						else if (guard is GuardCurrencyMerchant)
							guard.Inventory = KeepGuardInventoryMgr.Hibernia_Merchant.CloneTemplate();
						else if (guard is GuardHealer)
							guard.Inventory = KeepGuardInventoryMgr.Hibernia_Healer.CloneTemplate();
						else if (guard is GuardArcher)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Hibernia_ArcherPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Hibernia_Archer.CloneTemplate();
						}
						else if (guard is GuardCaster)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = KeepGuardInventoryMgr.Hibernia_CasterPK.CloneTemplate();
							else guard.Inventory = KeepGuardInventoryMgr.Hibernia_Caster.CloneTemplate();
						}
						else if (guard is GuardStealther)
							guard.Inventory = KeepGuardInventoryMgr.Hibernia_Stealther.CloneTemplate();
						break;
					}
			}
            ...
            else if ((guard is GuardFighter || guard is GuardCommander || guard is KeepLord || guard is GuardRelicKeepFighter) && UtilCollection.Chance(50))
				guard.SwitchWeapon(EActiveWeaponSlot.TwoHanded);
