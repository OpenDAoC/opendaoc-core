	...
	public class HibCuuldurachBrain : StandardMobBrain
	{
		...
		private int SpawnMssengers(ECSGameTimer timer)
		{
			for (int i = 0; i <= UtilCollection.Random(3, 5); i++)
			{
				...
				messenger.X = 408752 + UtilCollection.Random(-100, 100);
				messenger.Y = 706546 + UtilCollection.Random(-100, 100);
				messenger.Z = 2974;
...
namespace DOL.GS
{
	public class CuuldurachSpawnedAdd : GameNPC
	{
		...
		public override bool AddToWorld()
		{
			...
			switch (Name)
			{
				case "glimmer knight": Model = 2390; Size = (byte)UtilCollection.Random(50, 55); break;
				case "glimmer deathwatcher": Model = 2389; Size = (byte)UtilCollection.Random(50, 55); break;
				case "glimmer geist": Model = 2388; Size = (byte)UtilCollection.Random(50, 55); break;
			}
