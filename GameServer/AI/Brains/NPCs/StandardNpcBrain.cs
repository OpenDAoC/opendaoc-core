...
namespace DOL.AI.Brain
{
    ...
    /// </summary>
    public class StandardNpcBrain : PlayerVicinityBrain, IOldAggressiveBrain
    {
        ...
        /// </summary>
        public StandardNpcBrain() : base()
        {
            FSM = new FsmBase();
            FSM.Add(new StandardNpcState_Idle(this));
            FSM.Add(new StandardNpcState_WakingUp(this));
            FSM.Add(new StandardNpcState_Aggro(this));
            FSM.Add(new StandardNpcState_ReturnToSpawn(this));
            FSM.Add(new StandardNpcState_Patrolling(this));
            FSM.Add(new StandardNpcState_Roaming(this));
        ...
        protected virtual void CheckNPCAggro()
        {
            foreach (GameNPC npc in Body.GetNPCsInRadius((ushort) AggroRange))
            {
                ...
                if (Properties.ALWAYS_CHECK_LOS)
                {
                    ...
                    if (npc.Brain is NpcControlledBrain theirControlledNpcBrain && theirControlledNpcBrain.GetPlayerOwner() is GamePlayer theirOwner)
                    {
                        theirOwner.Out.SendCheckLOS(Body, npc, new CheckLOSResponse(LosCheckForAggroCallback));
                        continue;
                    }
                    else if (this is NpcControlledBrain ourControlledNpcBrain && ourControlledNpcBrain.GetPlayerOwner() is GamePlayer ourOwner)
                    {
        ...
        /// </summary>
        public void AddAggroListTo(StandardNpcBrain brain)
        {
        ...
        {
            if (attacker is GameNPC NpcAttacker && NpcAttacker.Brain is NpcControlledBrain controlledBrain)
            {
            ...
            if (maxAdds > 0)
            {
                ...
                while (numAdds < maxAdds && range <= BAFMaxRange)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(range))
                    {
                        ...
                        if (npc.IsFriend(Body) && npc.IsAggressive && npc.IsAvailable && npc.Brain is StandardNpcBrain brain)
                        {
