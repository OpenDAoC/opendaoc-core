...
namespace DOL.GS.Commands
{
	...
	public class CraftCommand : AbstractCommandHandler, ICommandHandler
	{
		public void OnCommand(GameClient client, string[] args)
		{
			...
			try
			{
				...
				if (args[1].ToLower() == "salvageinfo")
				{
					...
					DbSalvageYields salvage = GameServer.Database.FindObjectByKey<DbSalvageYields>(salvageID);
					...
					DbItemTemplates template = GameServer.Database.FindObjectByKey<DbItemTemplates>(salvage.MaterialId_nb);
				...
				if (args[1].ToLower() == "adjustprices")
				{
					...
					var DBrecipes = GameServer.Database.SelectAllObjects<DbCraftedItems>();
				...
				if (args[1].ToLower() == "salvageadd" || args[1].ToLower() == "salvageupdate")
				{
					try
					{
						...
						DbItemTemplates template = GameServer.Database.FindObjectByKey<DbItemTemplates>(material);
						...
						else
						{
							...
							if (salvage.PackageID == DbSalvageYields.LEGACY_SALVAGE_ID)
							{
