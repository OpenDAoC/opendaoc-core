...
namespace DOL.GS.Commands
{
    ...
    public class NggCommand : AbstractCommandHandler, ICommandHandler
    {
        /*
           armor[0][][] = cloth;
           armor[1][][] = leather;
        ...
        */
        ...
        public static void OnScriptCompiled(CoreEvent e, object sender, EventArgs args)
        {
            ...
            for (int a = 0; a < 6; a++)
            {
                ...
                for (int b = 0; b < 6; b++)
                {
                   ...
                   temp = GameServer.Database.SelectObjects<DbItemTemplates>(DB.Column("Object_Type").IsEqualTo(ot[a]).And(DB.Column("Item_Type").IsEqualTo(slots[b])));
                  	armor[a][b] = new ushort[temp.Count];
                    foreach (DataObject item in temp)
                    {
                        armor[a][b][x++] = (ushort)(item as DbItemTemplates).Model;
                    }
                }
            }

            x = 0;
            temp = GameServer.Database.SelectObjects<DbItemTemplates>(DB.Column("Item_Type").IsEqualTo(slots[6]));
            cloak = new ushort[temp.Count];
            foreach (DataObject item in temp)
            {
                cloak[x++] = (ushort)(item as DbItemTemplates).Model;
            }

            for (int a = 0; a < 11; a++)
            {
                x = 0;
                temp = GameServer.Database.SelectObjects<DbItemTemplates>(DB.Column("Item_Type").IsEqualTo(slots[a]));
                equip[a] = new ushort[temp.Count];
                foreach (DataObject item in temp)
                {
                    equip[a][x++] = (ushort)(item as DbItemTemplates).Model;
                }
            }
        }
        ...
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            else
            {
                ...
                DbItemTemplates tgeneric0 = (DbItemTemplates)GameServer.Database.FindObjectByKey<DbItemTemplates> (ClasseEpic + "EpicHelm");
                DbItemTemplates tgeneric1 = (DbItemTemplates)GameServer.Database.FindObjectByKey<DbItemTemplates> (ClasseEpic + "EpicGloves");
                DbItemTemplates tgeneric2 = (DbItemTemplates)GameServer.Database.FindObjectByKey<DbItemTemplates> (ClasseEpic + "EpicArms");
                DbItemTemplates tgeneric3 = (DbItemTemplates)GameServer.Database.FindObjectByKey<DbItemTemplates> (ClasseEpic + "EpicVest");
                DbItemTemplates tgeneric4 = (DbItemTemplates)GameServer.Database.FindObjectByKey<DbItemTemplates> (ClasseEpic + "EpicLegs");
                DbItemTemplates tgeneric5 = (DbItemTemplates)GameServer.Database.FindObjectByKey<DbItemTemplates> (ClasseEpic + "EpicBoots");
