...
namespace DOL.GS.Commands
{
    ...
    public class ZoneCommand : AbstractCommandHandler, ICommandHandler
    {
        public void OnCommand(GameClient client, string[] args)
        {
			try
			{
				...
				if (args[1].ToLower() == "info")
				{
					...
					var dbZone = CoreDb<DbZones>.SelectObject(DB.Column("ZoneID").IsEqualTo(zone.ID).And(DB.Column("RegionID").IsEqualTo(zone.ZoneRegion.ID)));
