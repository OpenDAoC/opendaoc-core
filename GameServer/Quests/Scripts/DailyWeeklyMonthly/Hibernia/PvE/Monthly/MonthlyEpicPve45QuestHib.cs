...
{
	public class MonthlyEpicPve45QuestHib : Quests.MonthlyQuest
	{
		/// <summary>
		...
		// Constructors
		public MonthlyEpicPve45QuestHib() : base()
		{
		}

		public MonthlyEpicPve45QuestHib(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public MonthlyEpicPve45QuestHib(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public MonthlyEpicPve45QuestHib(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			Anthony.AddQuestToGive(typeof (MonthlyEpicPve45QuestHib));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			Anthony.RemoveQuestToGive(typeof (MonthlyEpicPve45QuestHib));
		}
		...
		private static void TalkToAnthony(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(Anthony.CanGiveQuest(typeof (MonthlyEpicPve45QuestHib), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			MonthlyEpicPve45QuestHib quest = player.IsDoingQuest(typeof (MonthlyEpicPve45QuestHib)) as MonthlyEpicPve45QuestHib;
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						case "kill Balor and Myrddraxis":
							player.Out.SendQuestSubscribeCommand(Anthony, QuestMgr.GetIDForQuestType(typeof(MonthlyEpicPve45QuestHib)), "Will you help Anthony "+questTitle+"?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (MonthlyEpicPve45QuestHib)) != null)
				return true;
		...
		private static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(MonthlyEpicPve45QuestHib)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!Anthony.GiveQuest(typeof (MonthlyEpicPve45QuestHib), player, 1))
					return;
		...
		public override void FinishQuest()
		{
			if (m_questPlayer.Inventory.IsSlotsFree(3, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
			{
				...
				RogMgr.GenerateReward(m_questPlayer, 3000);
