...
{
	public class FrontiersMonthly45QuestHib : Quests.MonthlyQuest
	{
		/// <summary>
		...
		// Constructors
		public FrontiersMonthly45QuestHib() : base()
		{
		}

		public FrontiersMonthly45QuestHib(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public FrontiersMonthly45QuestHib(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public FrontiersMonthly45QuestHib(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			Kelteen.AddQuestToGive(typeof (FrontiersMonthly45QuestHib));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			Kelteen.RemoveQuestToGive(typeof (FrontiersMonthly45QuestHib));
		}
		...
		private static void TalkToKelteen(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(Kelteen.CanGiveQuest(typeof (FrontiersMonthly45QuestHib), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			FrontiersMonthly45QuestHib quest = player.IsDoingQuest(typeof (FrontiersMonthly45QuestHib)) as FrontiersMonthly45QuestHib;
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						case "order and security":
							player.Out.SendQuestSubscribeCommand(Kelteen, QuestMgr.GetIDForQuestType(typeof(FrontiersMonthly45QuestHib)), "Will you help "+Kelteen.Name+" to slay enemies and capture keeps? " + questTitle + "?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (FrontiersMonthly45QuestHib)) != null)
				return true;
		...
		private static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(FrontiersMonthly45QuestHib)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!Kelteen.GiveQuest(typeof (FrontiersMonthly45QuestHib), player, 1))
					return;
		...
		public override void Notify(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (player == null || player.IsDoingQuest(typeof(FrontiersMonthly45QuestHib)) == null)
				return;
