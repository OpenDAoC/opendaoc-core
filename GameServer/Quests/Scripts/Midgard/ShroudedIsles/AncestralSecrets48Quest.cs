...
{
	public class AncestralSecrets48Quest : BaseQuest
	{
		/// <summary>
		...
		private static DbItemTemplates beaded_resisting_stone;
		private static DbItemTemplates stone_pendant;
		private static DbItemTemplates quest_pendant;
		
		// Constructors
		public AncestralSecrets48Quest() : base()
		{
		}

		public AncestralSecrets48Quest(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public AncestralSecrets48Quest(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public AncestralSecrets48Quest(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
        ...
        // end npc
			...
			#region defineItems
			beaded_resisting_stone = GameServer.Database.FindObjectByKey<DbItemTemplates>("Beaded Resisting Stones");

			quest_pendant = GameServer.Database.FindObjectByKey<DbItemTemplates>("quest_pendant");
	        if (quest_pendant == null)
	        {
		        if (log.IsWarnEnabled)
			        log.Warn("Could not find Lightly Decorated Pendant, creating it ...");
		        quest_pendant = new DbItemTemplates();
	        ...
	        stone_pendant = GameServer.Database.FindObjectByKey<DbItemTemplates>("stone_pendant");
	        if (stone_pendant == null)
	        {
		        if (log.IsWarnEnabled)
			        log.Warn("Could not find Stone Pendant, creating it ...");
		        stone_pendant = new DbItemTemplates();
			...
			OtaYrling?.AddQuestToGive(typeof (AncestralSecrets48Quest));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			OtaYrling.RemoveQuestToGive(typeof (AncestralSecrets48Quest));
		}
		...
		{
			var quest = player.IsDoingQuest(typeof(AncestralSecrets48Quest)) as AncestralSecrets48Quest;
		...
		protected static void TalkToOtaYrling(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(OtaYrling.CanGiveQuest(typeof (AncestralSecrets48Quest), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			AncestralSecrets48Quest quest = player.IsDoingQuest(typeof (AncestralSecrets48Quest)) as AncestralSecrets48Quest;
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					{
						case "Curse":
							player.Out.SendQuestSubscribeCommand(OtaYrling, QuestMgr.GetIDForQuestType(typeof(AncestralSecrets48Quest)), "Will you help Ota Yrling find [Ancestral Secrets]?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (AncestralSecrets48Quest)) != null)
				return true;
		...
		protected static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(AncestralSecrets48Quest)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!OtaYrling.GiveQuest(typeof (AncestralSecrets48Quest), player, 1))
					return;
			}
		...
		public override void FinishQuest()
		{
			if (m_questPlayer.Inventory.IsSlotsFree(1, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
			{
				...
				m_questPlayer.AddMoney(MoneyUtil.GetMoney(0, 0, 121, 41, UtilCollection.Random(50)), "You receive {0} as a reward.");
