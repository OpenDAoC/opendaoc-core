...
{
	public class ThidKeepCaptureAlb20Quest : Quests.DailyQuest
	{
		/// <summary>
		...
		// Constructors
		public ThidKeepCaptureAlb20Quest() : base()
		{
		}

		public ThidKeepCaptureAlb20Quest(GamePlayer questingPlayer) : base(questingPlayer, 1)
		{
		}

		public ThidKeepCaptureAlb20Quest(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public ThidKeepCaptureAlb20Quest(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			PazzAlb.AddQuestToGive(typeof (ThidKeepCaptureAlb20Quest));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			PazzAlb.RemoveQuestToGive(typeof (ThidKeepCaptureAlb20Quest));
		}
		...
		private static void TalkToHaszan(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(PazzAlb.CanGiveQuest(typeof (ThidKeepCaptureAlb20Quest), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			ThidKeepCaptureAlb20Quest quest = player.IsDoingQuest(typeof (ThidKeepCaptureAlb20Quest)) as ThidKeepCaptureAlb20Quest;
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						case "help a skeleton":
							player.Out.SendQuestSubscribeCommand(PazzAlb, QuestMgr.GetIDForQuestType(typeof(ThidKeepCaptureAlb20Quest)), "Will you help Pazz with "+questTitle+"");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (ThidKeepCaptureAlb20Quest)) != null)
				return true;
		...
		protected static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(ThidKeepCaptureAlb20Quest)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!PazzAlb.GiveQuest(typeof (ThidKeepCaptureAlb20Quest), player, 1))
					return;
