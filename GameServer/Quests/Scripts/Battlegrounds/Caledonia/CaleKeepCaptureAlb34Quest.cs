...
{
	public class CaleKeepCaptureAlb34Quest : Quests.DailyQuest
	{
		/// <summary>
		...
		// Constructors
		public CaleKeepCaptureAlb34Quest() : base()
		{
		}

		public CaleKeepCaptureAlb34Quest(GamePlayer questingPlayer) : base(questingPlayer, 1)
		{
		}

		public CaleKeepCaptureAlb34Quest(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public CaleKeepCaptureAlb34Quest(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			PazzAlb.AddQuestToGive(typeof (CaleKeepCaptureAlb34Quest));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			PazzAlb.RemoveQuestToGive(typeof (CaleKeepCaptureAlb34Quest));
		}
		...
		private static void TalkToHaszan(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(PazzAlb.CanGiveQuest(typeof (CaleKeepCaptureAlb34Quest), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			CaleKeepCaptureAlb34Quest quest = player.IsDoingQuest(typeof (CaleKeepCaptureAlb34Quest)) as CaleKeepCaptureAlb34Quest;
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						case "help a skeleton":
							player.Out.SendQuestSubscribeCommand(PazzAlb, QuestMgr.GetIDForQuestType(typeof(CaleKeepCaptureAlb34Quest)), "Will you help Pazz with "+questTitle+"");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (CaleKeepCaptureAlb34Quest)) != null)
				return true;
		...
		protected static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(CaleKeepCaptureAlb34Quest)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!PazzAlb.GiveQuest(typeof (CaleKeepCaptureAlb34Quest), player, 1))
					return;
