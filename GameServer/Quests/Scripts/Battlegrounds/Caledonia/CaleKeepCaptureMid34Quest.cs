...
{
	public class CaleKeepCaptureMid34Quest : Quests.DailyQuest
	{
		/// <summary>
		...
		// Constructors
		public CaleKeepCaptureMid34Quest() : base()
		{
		}

		public CaleKeepCaptureMid34Quest(GamePlayer questingPlayer) : base(questingPlayer, 1)
		{
		}

		public CaleKeepCaptureMid34Quest(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public CaleKeepCaptureMid34Quest(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			PazzMid.AddQuestToGive(typeof (CaleKeepCaptureMid34Quest));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			PazzMid.RemoveQuestToGive(typeof (CaleKeepCaptureMid34Quest));
		}
		...
		private static void TalkToHerou(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(PazzMid.CanGiveQuest(typeof (CaleKeepCaptureMid34Quest), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			CaleKeepCaptureMid34Quest quest = player.IsDoingQuest(typeof (CaleKeepCaptureMid34Quest)) as CaleKeepCaptureMid34Quest;
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						case "help a skeleton":
							player.Out.SendQuestSubscribeCommand(PazzMid, QuestMgr.GetIDForQuestType(typeof(CaleKeepCaptureMid34Quest)), "Will you help Pazz with "+questTitle+"");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (CaleKeepCaptureMid34Quest)) != null)
				return true;
		...
		private static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(CaleKeepCaptureMid34Quest)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!PazzMid.GiveQuest(typeof (CaleKeepCaptureMid34Quest), player, 1))
					return;
