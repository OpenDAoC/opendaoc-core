...
{
	public class TheLostSeed48Quest : BaseQuest
	{
		/// <summary>
		...
		private static HibSiTeleporter Emolia = null; // step 3
		...
		private static DbItemTemplates paidrean_necklace;
		private static DbItemTemplates glowing_red_jewel;
		// Constructors
		public TheLostSeed48Quest() : base()
		{
		}

		public TheLostSeed48Quest(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public TheLostSeed48Quest(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public TheLostSeed48Quest(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
        ...
        // end npc
			...
			#region defineItems
			paidrean_necklace = GameServer.Database.FindObjectByKey<DbItemTemplates>("Paidrean Necklace");

	        glowing_red_jewel = GameServer.Database.FindObjectByKey<DbItemTemplates>("glowing_red_jewel");
	        if (glowing_red_jewel == null)
	        {
		        if (log.IsWarnEnabled)
			        log.Warn("Could not find Glowing Red Jewel, creating it ...");
		        glowing_red_jewel = new DbItemTemplates();
			...
			Terod?.AddQuestToGive(typeof (TheLostSeed48Quest));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			Terod.RemoveQuestToGive(typeof (TheLostSeed48Quest));
		}
		...
		{
			var quest = player.IsDoingQuest(typeof(TheLostSeed48Quest)) as TheLostSeed48Quest;
		...
		protected static void TalkToTerod(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(Terod.CanGiveQuest(typeof (TheLostSeed48Quest), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			TheLostSeed48Quest quest = player.IsDoingQuest(typeof (TheLostSeed48Quest)) as TheLostSeed48Quest;
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						...
						case "Lost Seed":
							player.Out.SendQuestSubscribeCommand(Terod, QuestMgr.GetIDForQuestType(typeof(TheLostSeed48Quest)), "Will you help Terod find [The Lost Seed]?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (TheLostSeed48Quest)) != null)
				return true;
		...
		protected static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(TheLostSeed48Quest)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!Terod.GiveQuest(typeof (TheLostSeed48Quest), player, 1))
					return;
			}
