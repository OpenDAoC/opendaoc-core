...
namespace DOL.GS.Effects
{
    ...
    public class NfRaSoldiersCitadelEffect : TimedEffect
    {
        ...
        public NfRaSoldiersCitadelEffect()
            : base(RealmAbilities.NfRaSoldiersCitadelHandler.DURATION)
        { }
        ...
        public override void Stop()
        {
            if (EffectOwner != null)
            {
                EffectOwner.BaseBuffBonusCategory[(int)EProperty.ParryChance] -= 50;
                EffectOwner.BaseBuffBonusCategory[(int)EProperty.BlockChance] -= 50;
                if(EffectOwner.IsAlive) new NfRaSoldiersCitadelSecondaryEffect().Start(EffectOwner);
    ...
    public class NfRaSoldiersCitadelSecondaryEffect : TimedEffect
    {
        private GamePlayer EffectOwner;

        public NfRaSoldiersCitadelSecondaryEffect()
            : base(RealmAbilities.NfRaSoldiersCitadelHandler.SECOND_DURATION)
        { }
        ...
        private static void PlayerLeftWorld(CoreEvent e, object sender, EventArgs args)
        {
			...
			NfRaSoldiersCitadelSecondaryEffect soldiersCitadel = player.EffectList.GetOfType<NfRaSoldiersCitadelSecondaryEffect>();
            if (soldiersCitadel != null)
                soldiersCitadel.Cancel(false);
       }
