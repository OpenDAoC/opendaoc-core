...
namespace DOL.GS.Effects
{
    public class OfRaVolleyEcsEffect : EcsGameAbilityEffect
    {
        ...
        public OfRaVolleyEcsEffect(ECSGameEffectInitParams initParams) : base(initParams)
        {
        ...
        public void DecideNextShoot()
        {
            ...
            if (_remainingShots == 0)
            {
                ...
                Cancel(false);
                OfRaVolleyHandler volley = OwnerPlayer.GetAbility<OfRaVolleyHandler>();
                OwnerPlayer.DisableSkill(volley, OfRaVolleyHandler.DISABLE_DURATION);
            }
        }
        ...
        private void PlayerUseVolley(CoreEvent e, object sender, EventArgs args)
        {
            ...
            if (player.IsWithinRadius(player.GroundTarget, OfRaVolleyHandler.GetMinAttackRange(player.Realm)))
            {
                player.Out.SendMessage("You ground target is too close to use Volley!", EChatType.CT_System, EChatLoc.CL_SystemWindow);
                return;
            }

            if (!player.IsWithinRadius(player.GroundTarget, OfRaVolleyHandler.GetMaxAttackRange(player.Realm)))
            {
        ...
        private int MakeAttack(ECSGameTimer timer)
        {
            ...
            attackComponent.weaponAction = new WeaponAction(OwnerPlayer, potentialTargets[UtilCollection.Random(0, potentialTargets.Count - 1)], weaponActionData.AttackWeapon, 1.0, weaponActionData.InterruptDuration, ERangedAttackType.Volley);
