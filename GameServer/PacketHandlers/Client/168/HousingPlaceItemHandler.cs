...
{
	[PacketHandlerAttribute(EPacketHandlerType.TCP, EClientPackets.HousePlaceItem, "Handles things like placing indoor/outdoor items.", eClientStatus.PlayerInGame)]
	public class HousingPlaceItemHandler : IPacketHandler
	{
					...
					case 1: // GARDEN OBJECT
						{
							...
							if (house.OutdoorItems.Count >= ServerProperties.ServerProperties.MAX_OUTDOOR_HOUSE_ITEMS)
							{
							...
							ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.GardenItemPlaced",
													   ServerProperties.ServerProperties.MAX_OUTDOOR_HOUSE_ITEMS - house.OutdoorItems.Count);
					...
					case 5: // HOOKPOINT
						{
							...
							else if (house.GetHookpointLocation((uint)_position) != null)
							{
								...
								foreach (var hpitem in CoreDb<DbHouseHookPointItems>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber)))
								{
		...
		{
			int maxitems = ServerProperties.ServerProperties.MAX_INDOOR_HOUSE_ITEMS;

			if (ServerProperties.ServerProperties.INDOOR_ITEMS_DEPEND_ON_SIZE)
			{
				switch (model)
				{
					case 1:
