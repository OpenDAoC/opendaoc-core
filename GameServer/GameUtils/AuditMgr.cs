...
namespace DOL.GS
{
	...
	public static class AuditMgr
	{
		...
		private static List<DbAuditEntries> _queuedAuditEntries;
		private static SpinWaitLock _updateLock;

		static AuditMgr()
		{
			_queuedAuditEntries = new List<DbAuditEntries>();
		...
		private static void OnPushTimerElapsed(object sender, ElapsedEventArgs e)
		{
			...
			foreach (DbAuditEntries entry in oldQueue)
			{
		...
		private static StringBuilder GetEntryQueryBuilder()
		{
			...
			string tableName = AttributesUtils.GetTableOrViewName(typeof (DbAuditEntries));
		...
		public static void AddAuditEntry(int type, int subType, string oldValue, string newValue)
		{
			...
			var transactionHistory = new DbAuditEntries
			                         	{
