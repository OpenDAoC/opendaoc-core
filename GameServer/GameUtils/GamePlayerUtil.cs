	...
	public static class GamePlayerUtil
	{
		...
		public static string GetTranslatedSpotDescription(this Region reg, GameClient client, int x, int y, int z)
		{
			if (reg != null)
			{
				...
				if (area != null)
				{
					var lng = LanguageMgr.GetTranslation(client, area) as DbLanguageAreas;
					
					if (lng != null && !UtilCollection.IsEmpty(lng.ScreenDescription))
				...
				if (zone != null)
				{
					var lng = LanguageMgr.GetTranslation(client, zone) as DbLanguageZones;
					if (lng != null)
						return lng.ScreenDescription;
