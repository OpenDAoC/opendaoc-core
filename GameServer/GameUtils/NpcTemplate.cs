...
namespace DOL.GS
{
	...
	public class NpcTemplate : INpcTemplate
	{
		...
		public NpcTemplate(DBNpcTemplate data)
		{
			...
			{
				foreach (string splitab in UtilCollection.SplitCSV(data.Abilities))
				{
					...
					if (ab.Length>1)
						id = int.Parse(ab[1]);
					AbilityUtil abil = SkillBase.GetAbility(ab[0], id);
					if (abil != null)
		...
		public NpcTemplate( GameNPC mob )
		{
			...
			if ( mob.Abilities != null && mob.Abilities.Count > 0 )
			{
				try
				{
					...
					foreach (AbilityUtil mobAbility in mob.Abilities.Values)
					{
		...
		public virtual void SaveIntoDatabase()
		{
			...
			if( m_abilities != null && m_abilities.Count > 0 )
			{
				...
				foreach ( AbilityUtil npcAbility in m_abilities )
				{
					if ( npcAbility != null )
