...
namespace DOL.GS
{
    ...
    public class LootGeneratorTemplate : LootGeneratorBase
    {
        ...
        /// </summary>
        protected static Dictionary<string, Dictionary<string, DbLootTemplates>> m_lootTemplates = null;

        /// <summary>
        /// Map holding the corresponding LootTemplateName for each MobName
        /// 1:n Mapping between Mob and LootTemplate
        /// </summary>
        protected static Dictionary<string, List<DbMobXLootTemplates>> m_mobXLootTemplates = null;
        ...
        protected static bool PreloadLootTemplates()
        {
            ...
            {
                m_lootTemplates = new Dictionary<string, Dictionary<string, DbLootTemplates>>();
                Dictionary<string, DbItemTemplates> itemtemplates = new Dictionary<string, DbItemTemplates>();

                lock (m_lootTemplates)
                {
                    IList<DbLootTemplates> dbLootTemplates = null;

                    try
                    {
                        // TemplateName (typically the mob name), ItemTemplateID, Chance
                        dbLootTemplates = GameServer.Database.SelectAllObjects<DbLootTemplates>();
                        itemtemplates = GameServer.Database.SelectAllObjects<DbItemTemplates>()
                    ...
                    {
                        Dictionary<string, DbLootTemplates> loot = null;

                        foreach (DbLootTemplates dbTemplate in dbLootTemplates)
                        {
                            if (!m_lootTemplates.TryGetValue(dbTemplate.TemplateName.ToLower(), out loot))
                            {
                                loot = new Dictionary<string, DbLootTemplates>();
                                m_lootTemplates[dbTemplate.TemplateName.ToLower()] = loot;
                            }

                            DbItemTemplates drop = null;
            ...
            {
                m_mobXLootTemplates = new Dictionary<string, List<DbMobXLootTemplates>>();

                lock (m_mobXLootTemplates)
                {
                    IList<DbMobXLootTemplates> dbMobXLootTemplates = null;

                    try
                    {
                        // MobName, LootTemplateName, DropCount
                        dbMobXLootTemplates = GameServer.Database.SelectAllObjects<DbMobXLootTemplates>();
                    }
                    catch (Exception e)
                    {
                        if (log.IsErrorEnabled)
        ...
        public override void Refresh(GameNPC mob)
        {
            ...
            var mxlts = CoreDb<DbMobXLootTemplates>.SelectObjects(DB.Column("MobName").IsEqualTo(mob.Name.ToLower()));

            if (mxlts != null)
            {
                lock (m_mobXLootTemplates)
                {
                    foreach (DbMobXLootTemplates mxlt in mxlts)
                    {
                        List<DbMobXLootTemplates> mobxLootTemplates;
        ...
        protected void RefreshLootTemplate(string templateName)
        {
            ...
            var lootTemplates =
                CoreDb<DbLootTemplates>.SelectObjects(DB.Column("TemplateName").IsEqualTo(templateName.ToLower()));
            ...
            if (lootTemplates != null)
            {
                lock (m_lootTemplates)
                {
                    ...
                    Dictionary<string, DbLootTemplates> lootList = new Dictionary<string, DbLootTemplates>();

                    foreach (DbLootTemplates lt in lootTemplates)
                    {
        ...
        public override LootList GenerateLoot(GameNPC mob, GameObject killer)
        {
            ...
            try
            {
                ...
                if (player != null)
                {
                    List<DbMobXLootTemplates> killedMobXLootTemplates = null;
                    ...
                    if (killedMobXLootTemplates == null)
                    {
                        ...
                        {
                            Dictionary<string, DbLootTemplates> lootTemplatesToDrop = m_lootTemplates[mob.Name.ToLower()];
                            List<DbLootTemplates> timedDrops = new List<DbLootTemplates>();

                            if (lootTemplatesToDrop != null)
                            {
                                long dropChan = 0;
                                long tmp = 0;
                                foreach (DbLootTemplates lootTemplate in lootTemplatesToDrop.Values)
                                {
                                    DbItemTemplates drop =
                                        GameServer.Database.FindObjectByKey<DbItemTemplates>(lootTemplate.ItemTemplateID);
                                ...
                                if (timedDrops.Count > 0)
                                {
                                    DbLootTemplates
                                        lootTemplate =
                                            timedDrops[
                                                UtilCollection.Random(timedDrops.Count - 1)]; //randomly pick one available drop

                                    lock (player._xpGainersLock)
                                    {
                                        DbItemTemplates drop =
                                            GameServer.Database.FindObjectByKey<DbItemTemplates>(lootTemplate
                                                .ItemTemplateID);
                                        int dropCooldown =
                    ...
                    else
                    {
                        ...
                        List<DbLootTemplates> lootTemplatesToDrop = new List<DbLootTemplates>();

                        long dropChan = 0;
                        long tmp = 0;
                        foreach (DbMobXLootTemplates mobXLootTemplate in killedMobXLootTemplates)
                        {
                            ...
                            {
                                List<DbLootTemplates> timedDrops = new List<DbLootTemplates>();
                                foreach (DbLootTemplates lootTemplate in lootTemplatesToDrop)
                                {
                                    DbItemTemplates drop =
                                        GameServer.Database.FindObjectByKey<DbItemTemplates>(lootTemplate.ItemTemplateID);
        ...
        private LootList GenerateLootFromMobXLootTemplates(DbMobXLootTemplates mobXLootTemplates,
            List<DbLootTemplates> lootTemplates, LootList lootList, GamePlayer player)
        {
            ...
            Dictionary<string, DbLootTemplates> templateList = null;
            ...
            {
                foreach (DbLootTemplates lootTemplate in templateList.Values)
                {
