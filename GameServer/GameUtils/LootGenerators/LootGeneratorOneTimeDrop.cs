...
namespace DOL.GS
{
	...
	public class LootGeneratorOneTimeDrop : LootGeneratorBase
	{
		...
		protected static bool PreloadLootOTDs()
		{
			lock (m_mobOTDList)
			{
				...
				if (lootOTDs != null && lootOTDs.Count > 0)
				{
					...
					{
						IList<DbMobs> mobs = CoreDb<DbMobs>.SelectObjects(DB.Column("Name").IsEqualTo(l.MobName));
		...
		public override void Refresh(GameNPC mob)
		{
			...
			IList<DbLootOtds> otds = CoreDb<DbLootOtds>.SelectObjects(DB.Column("MobName").IsEqualTo(mob.Name));
