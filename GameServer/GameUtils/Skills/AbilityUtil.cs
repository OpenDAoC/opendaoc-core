...
namespace DOL.GS
{
	...
	{
		void Execute(AbilityUtil ab, GamePlayer player);
	}
	...
	/// </summary>
	public class AbilityUtil : NamedSkill
	{
		...
		public AbilityUtil(DbAbilities dba)
			: this(dba, 0)
		{
		}

		public AbilityUtil(DbAbilities dba, int level)
			: this(dba.KeyName, dba.Name, dba.Description, dba.AbilityID, (ushort)dba.IconID, level, dba.InternalID)
		{
		}

		public AbilityUtil(DbAbilities dba, int level, string spec, int speclevel)
			: this(dba.KeyName, dba.Name, dba.Description, dba.AbilityID, (ushort)dba.IconID, level, spec, speclevel, dba.InternalID)
		{
		}

		public AbilityUtil(string keyname, string displayname, string description, int id, ushort icon, int level, int internalID)
			: this(keyname, displayname, description, id, icon, level, "", 0, internalID)
		{
		}

		public AbilityUtil(string keyname, string displayname, string description, int id, ushort icon, int level, string spec, int speclevel, int internalID)
		...
		private void UpdateCurrentName()
		{
			try
			{
				...
				foreach (string levelNamePair in UtilCollection.SplitCSV(name.Trim()))
				{
