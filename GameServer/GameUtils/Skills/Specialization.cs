...
namespace DOL.GS
{
	...
	public class Specialization : NamedSkill
	{
		...
		/// <param name="living"></param>
		/// <returns></returns>
		public virtual List<AbilityUtil> GetAbilitiesForLiving(GameLiving living)
		{
		...
		/// <param name="step">step is only used when called for pretending some level (for trainer display)</param>
		/// <returns></returns>
		public virtual List<AbilityUtil> PretendAbilitiesForLiving(GameLiving living, int step)
		{
		...
		/// <param name="level">level is only used when called for pretending some level (for trainer display)</param>
		/// <returns></returns>
		protected virtual List<AbilityUtil> GetAbilitiesForLiving(GameLiving living, int level)
		{
			// Select only Enabled and Max Level Abilities
			List<AbilityUtil> abs = SkillBase.GetSpecAbilityList(KeyName, living is GamePlayer ? ((GamePlayer)living).CharacterClass.ID : 0);
			
			// Get order of first appearing skills
			IOrderedEnumerable<AbilityUtil> order = abs.GroupBy(item => item.KeyName)
