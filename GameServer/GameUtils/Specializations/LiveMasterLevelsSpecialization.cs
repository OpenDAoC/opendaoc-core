...
namespace DOL.GS
{
	...
	{		
		...
		/// <returns></returns>
		protected override List<AbilityUtil> GetAbilitiesForLiving(GameLiving living, int level)
		{
			return new List<AbilityUtil>();
		}
		...
		protected override IDictionary<SpellLine, List<Skill>> GetLinesSpellsForLiving(GameLiving living, int level)
		{
			...
			if (player != null)
			{
				...
				if (spells.Keys.Count > 0)
				{
					...
					foreach (AbilityUtil ab in base.GetAbilitiesForLiving(player, level))
						list.Value.Add(ab);
