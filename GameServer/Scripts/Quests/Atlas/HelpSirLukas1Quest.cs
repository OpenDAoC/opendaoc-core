...
{
	public class HelpSirLukas1Quest : BaseQuest
	{
		/// <summary>
		...
		private static DbItemTemplates funeral_speech_scroll = null;
		private static DbItemTemplates FlitzitinaBow = null;

		private static DbWorldObjects FlitzitinasGrave = null;

		private static IList<DbWorldObjects> GetItems()
		{
			return GameServer.Database.SelectObjects<DbWorldObjects>(DB.Column("Name").IsEqualTo("Flitzitina's Grave"));
		}

		// Constructors
		public HelpSirLukas1Quest() : base()
		{
		}

		public HelpSirLukas1Quest(GamePlayer questingPlayer) : base(questingPlayer)
		{
		}

		public HelpSirLukas1Quest(GamePlayer questingPlayer, int step) : base(questingPlayer, step)
		{
		}

		public HelpSirLukas1Quest(GamePlayer questingPlayer, DbQuests dbQuest) : base(questingPlayer, dbQuest)
		{
		}
		...
		public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			#region defineItems
				...
				funeral_speech_scroll = GameServer.Database.FindObjectByKey<DbItemTemplates>("funeral_speech_flitzitina");
			if (funeral_speech_scroll == null)
			{
				if (log.IsWarnEnabled)
					log.Warn("Could not find Funeral Speech for Flitzitina, creating it ...");
				funeral_speech_scroll = new DbItemTemplates();
			...
			FlitzitinaBow = GameServer.Database.FindObjectByKey<DbItemTemplates>("FlitzitinaBow");
			if (FlitzitinaBow == null)
			{
				if (log.IsWarnEnabled)
					log.Warn("Could not find Flitzitina Bow , creating it ...");
				FlitzitinaBow = new DbItemTemplates();
			...
			if (graveCheck.Count == 0)
			{
				...
				var FlitzitinasGrave = new DbWorldObjects();
				...
				if (SAVE_INTO_DATABASE)
				{
					...
					HelpSirLukas1Quest.FlitzitinasGrave = FlitzitinasGrave;
				}
			...
			SirLukas.AddQuestToGive(typeof (HelpSirLukas1Quest));
		...
		public static void ScriptUnloaded(CoreEvent e, object sender, EventArgs args)
		{
			...
			SirLukas.RemoveQuestToGive(typeof (HelpSirLukas1Quest));
		}
		...
		protected static void TalkToSirLukas(CoreEvent e, object sender, EventArgs args)
		{
			...
			if(SirLukas.CanGiveQuest(typeof (HelpSirLukas1Quest), player)  <= 0)
				return;

			//We also check if the player is already doing the quest
			HelpSirLukas1Quest quest = player.IsDoingQuest(typeof (HelpSirLukas1Quest)) as HelpSirLukas1Quest;
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						case "support Camelot":
							player.Out.SendQuestSubscribeCommand(SirLukas, QuestMgr.GetIDForQuestType(typeof(HelpSirLukas1Quest)), "Will you help Sir Lukas [Memorial] All in the Gold]?");
							break;
					}
				}
				else
		...
		public override bool CheckQuestQualification(GamePlayer player)
		{
			...
			if (player.IsDoingQuest(typeof (HelpSirLukas1Quest)) != null)
				return true;
		...
		protected static void SubscribeQuest(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(HelpSirLukas1Quest)))
				return;
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				if (!SirLukas.GiveQuest(typeof (HelpSirLukas1Quest), player, 1))
					return;
		...
		public override void Notify(CoreEvent e, object sender, EventArgs args)
		{
			...
			if (player==null || player.IsDoingQuest(typeof (HelpSirLukas1Quest)) == null)
				return;
		...
		public class HelpSirLukasTitle : EventPlayerTitle 
    {
        ...
        {
	        return player.HasFinishedQuest(typeof(HelpSirLukas1Quest)) == 1;
        }
