...
namespace DOL.GS.GameEvents
{
	...
	/// </summary>
	public static class LevelUpRespecSettings
	{		
		/// <summary>
		...
		public static void OnLevelUp(CoreEvent e, object sender, EventArgs args)
		{
			...
			foreach (string str in UtilCollection.SplitCSV(GIVE_DOL_RESPEC_AT_LEVEL, true))
			{
				...
				if (player.Level == level_respec)
				{
                    ...
                    {
                        player.Out.SendMessage(string.Format("As you reached level {0}, you are awarded an Atlas (full) respec!", player.Level), EChatType.CT_Important, EChatLoc.CL_SystemWindow);
                    }
                }
			}
