        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            if (hasRealmCredit)
            {
                if (!HasCustomParam(player))
                {
                    if (player.Level < 50)
                    {
                        player.Out.SendMessage($"For your efforts, the realm of {RealmName(Realm)} has decided to award you with {ORBS_REWARD} Server Orbs, congratulations!", EChatType.CT_Say, EChatLoc.CL_PopupWindow);
        ...
        public override bool WhisperReceive(GameLiving source, string str)
        {
            ...
            switch(str)
            {
                case "receive them":
                    ...
                    player.Out.SendMessage($"Here, take {ORBS_REWARD} Server Orbs! \n\n" +
