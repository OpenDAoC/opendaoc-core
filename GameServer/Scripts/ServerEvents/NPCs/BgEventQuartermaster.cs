    ...
    public class BgEventQuartermaster : GameNpc
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			player.Out.SendMessage("Hello " + player.Name + "! We're happy to see you here, supporting your realm.\n" +
				...
				"Additionally, I can provide you with some [weapons], starting [coin], and an allotment of [Server Orbs].\n\n" +
		...
		public override bool WhisperReceive(GameLiving source, string str) {
			...
			} else if (str.Equals("gems"))
            {
				...
				{
					List<DbItemTemplates> serverGem = new List<DbItemTemplates>(CoreDb<DbItemTemplates>.SelectObjects(DB.Column("Id_nb").IsEqualTo("atlas_gem")));
					InventoryItem invitem = GameInventoryItem.Create<DbItemUnique>(serverGem.FirstOrDefault());
					player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, invitem);
				
					List<DbItemTemplates> serverCloak = new List<DbItemTemplates>(CoreDb<DbItemTemplates>.SelectObjects(DB.Column("Id_nb").IsEqualTo("atlas_cloak")));
					InventoryItem invitem2 = GameInventoryItem.Create<DbItemUnique>(serverCloak.FirstOrDefault());
					player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, invitem2);
					
					List<DbItemTemplates> serverRing = new List<DbItemTemplates>(CoreDb<DbItemTemplates>.SelectObjects(DB.Column("Id_nb").IsEqualTo("atlas_ring")));
					InventoryItem invitem3 = GameInventoryItem.Create<DbItemUnique>(serverRing.FirstOrDefault());
			...
			else if (str.Equals("Server Orbs"))
			{
				...
				if (hasFreeOrbs != null)
				{
					player.Out.SendMessage("Sorry " + player.Name + ", I don't have enough Server Orbs left to give you more.\n\n Go fight for your Realm to get some!", EChatType.CT_Say, EChatLoc.CL_PopupWindow);
					return false;
				}
