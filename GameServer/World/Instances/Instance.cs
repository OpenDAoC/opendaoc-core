...
namespace DOL.GS
{
	...
	public class Instance : BaseInstance
	{
		...
		{
			var objects = CoreDb<DbInstanceXElements>.SelectObjects(DB.Column("InstanceID").IsEqualTo(instanceName));
			...
			foreach (DbInstanceXElements entry in objects)
			{
				if (entry == null)
				...
				if ((GameNpc)obj != null && !UtilCollection.IsEmpty(entry.NPCTemplate, true))
				{
					...
					if (int.TryParse(listTemplate[UtilCollection.Random(listTemplate.Count-1)], out template) && template > 0)
					{	
