...
namespace DOL.GS
{
	...
	public class RegionInstance : BaseInstance
	{
		...
		/// </summary>
		public override void LoadFromDatabase(DbMobs[] mobObjs, ref long mobCount, ref long merchantCount, ref long itemCount, ref long bindCount)
        {
            if (!LoadObjects)
                return;
            ...
            if (mobObjs.Length > 0)
            {
                foreach (DbMobs mob in mobObjs)
                {
                    ...
                    if (myMob == null)
                    {
                    	if(template != null && template.ClassType != null && template.ClassType.Length > 0 && template.ClassType != DbMobs.DEFAULT_NPC_CLASSTYPE && template.ReplaceMobValues)
                    	{
                			classtype = template.ClassType;
                    	}
                        else if (mob.ClassType != null && mob.ClassType.Length > 0 && mob.ClassType != DbMobs.DEFAULT_NPC_CLASSTYPE)
                        {
            ...
            {
                foreach (DbWorldObjects item in staticObjs)
                {
                	DbWorldObjects itemclone = (DbWorldObjects)item.Clone();
            ...
            int areaCnt = 0;
            // Add missing area
            foreach(DbAreas area in areaObjs) 
            {
            	...
            	// clone DB object.
            	DbAreas newDBArea = ((DbAreas)area.Clone());
