...
{
	public class PygmyGoblinTanglerBrain : StandardNpcBrain
	{
			...
			if(HasAggro && Body.TargetObject != null)
            {
				if(!BringGoblins)
                {
					foreach(GameNPC npc in Body.GetNPCsInRadius(500))
                    {
						...
						if(npc != null && npc.IsAlive && npc.Name.ToLower() == "pygmy goblin" && npc.Brain is not NpcControlledBrain && npc.Brain is StandardNpcBrain brain)
                        {
