        ...
        public void PickNotRottedTarget()
        {
            ...
            else
            {
                if (Body.TargetObject != null)
                {
                    ...
                    RandomTarget = PlayersToAttack[UtilCollection.Random(0, PlayersToAttack.Count - 1)]; //mob next random target
...
namespace DOL.AI.Brain
{
    public class HurikaBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if(HasAggro && Body.TargetObject != null)
            {
                ...
                if(Port_Enemys.Count > 0)
                {
                    ...
                    {
                        new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(TeleportPlayer), UtilCollection.Random(15000, 20000));
                        IsTargetPicked = true;
                    }
                }
            }
        ...
        private int TeleportPlayer(ECSGameTimer timer)
        {
            ...
            {
                RandomTarget.MoveTo(Body.CurrentRegionID, Body.X, Body.Y, Body.Z + UtilCollection.Random(500, 700), Body.Heading);
...
namespace DOL.GS
{
    public class TorstEddies : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            Model = 665;
            Name = "eddie";
            Level = (byte)UtilCollection.Random(55, 58);
            Size = 50;
            RespawnInterval = -1;
...
namespace DOL.AI.Brain
{
    public class TorstEddiesBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            {
                Point3D oldPoint = new Point3D(TrostNpc.X + UtilCollection.Random(-200, 200), TrostNpc.Y + UtilCollection.Random(-200, 200), TrostNpc.Z + UtilCollection.Random(0, 100));
                Point3D newPoint = new Point3D(TrostNpc.X + UtilCollection.Random(-200, 200), TrostNpc.Y + UtilCollection.Random(-200, 200), TrostNpc.Z + UtilCollection.Random(0, 100));
        ...
        private Spell ColdGroundDD
        {
            get
            {
                if (m_ColdGroundDD == null)
                {
                    ...
                    spell.AllowAdd = false;
                    spell.CastTime = 0;
                    spell.RecastDelay = UtilCollection.Random(5,12);
