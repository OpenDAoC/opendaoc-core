    ...
    public class ConfusionEcsEffect : ECSGameSpellEffect
    {
        ...
        public override void OnStartEffect()
        {
            ...
            else if (Owner is GameNPC)
            {
                ...
                (SpellHandler as ConfusionHandler).targetList.Clear();
                foreach (GamePlayer target in npc.GetPlayersInRadius(1000))
                {
                    if (doAttackFriend)
                        (SpellHandler as ConfusionHandler).targetList.Add(target);
                    else
                    {
                ...
                if ((SpellHandler as ConfusionHandler).targetList.Count > 0)
                {
                    npc.StopAttack();
                    npc.StopCurrentSpellcast();

                    GameLiving target = (SpellHandler as ConfusionHandler).targetList[UtilCollection.Random((SpellHandler as ConfusionHandler).targetList.Count - 1)] as GameLiving;
