...
namespace DOL.GS
{
	...
	public class Group
	{
		...
		public virtual bool AddMember(GameLiving living) 
		{
			...
			var hasGrouped = CoreDb<DOLCharactersXCustomParam>.SelectObject(DB.Column("DOLCharactersObjectId").IsEqualTo(player.ObjectId).And(DB.Column("KeyName").IsEqualTo(customKey)));
		...
		public virtual bool RemoveMember(GameLiving living)
		{
			...
			if (player != null)
			{
				...
				foreach (GuardEcsEffect guard in living.effectListComponent.GetAbilityEffects().Where(x => x.EffectType == EEffect.Guard))
				{
