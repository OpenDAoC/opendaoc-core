					...
					for (int i = firstAccountSlot; i < firstAccountSlot + 10; i++)
					{
						...
						for (int j = 0; j < characters.Length && written == false; j++)
							if (characters[j].AccountSlot == i)
							{
								...
								items = CoreDb<InventoryItem>.SelectObjects(DB.Column("OwnerID").IsEqualTo(characters[j].ObjectId).And(DB.Column("SlotPosition").IsGreaterOrEqualTo(10)).And(DB.Column("SlotPosition").IsLessOrEqualTo(37)));
