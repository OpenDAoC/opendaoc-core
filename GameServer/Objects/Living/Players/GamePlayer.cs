...
namespace DOL.GS
{
    ...
    public class GamePlayer : GameLiving, IManagedEntity
    {
        ...
        /// Players class
        /// </summary>
        protected IPlayerClass m_characterClass;

        /// <summary>
        /// Gets the player's character class
        /// </summary>
        public virtual IPlayerClass CharacterClass
        {
        ...
        protected virtual int RespecSingleLine(Specialization specLine)
        {
            ...
            if (CharacterClass is ClassBonedancerOwner
        ...
        {
            return MoneyUtil.GetMoney(Mithril, Platinum, Gold, Silver, Copper);
        }
        ...
        public virtual void AddMoney(long money, string messageFormat, EChatType ct, EChatLoc cl)
        {
            ...
            Copper = MoneyUtil.GetCopper(newMoney);
            Silver = MoneyUtil.GetSilver(newMoney);
            Gold = MoneyUtil.GetGold(newMoney);
            Platinum = MoneyUtil.GetPlatinum(newMoney);
            Mithril = MoneyUtil.GetMithril(newMoney);

            Out.SendUpdateMoney();

            if (messageFormat != null)
            {
                Out.SendMessage(string.Format(messageFormat, MoneyUtil.GetString(money)), ct, cl);
            }
        }
                ...
                lock (moneyObject)
                {
                        ...
                        foreach (GamePlayer eligibleMember in gamePlayers)
                        {
                            if (eligibleMember.Guild != null && eligibleMember.Guild.IsGuildDuesOn())
                            {
                                ...
                                if (eligibleMember.Guild.GetGuildDuesPercent() != 100)
                                    eligibleMember.AddMoney(moneyToPlayer, LanguageMgr.GetTranslation(Client.Account.Language, "GamePlayer.PickupObject.YourLootShare", MoneyUtil.GetString(moneyToPlayer)));
                                else
                    ...
                    else
                    {
                        ...
                        if (Guild != null && Guild.IsGuildDuesOn() && moneyObject.TotalCopper > 0 && Guild.GetGuildDuesPercent() > 0)
                        {
                            ...
                            {
                                AddMoney(moneyObject.TotalCopper, LanguageMgr.GetTranslation(Client.Account.Language, "GamePlayer.PickupObject.YouPickUp", MoneyUtil.GetString(moneyObject.TotalCopper)));
                            }
                            else
                            {
        ...
        public GamePlayer(GameClient client, DbCoreCharacters dbChar) : base()
        {
            ...
            m_sitting = false;
            m_isWireframe = false;
            m_characterClass = new DefaultPlayerClass();
            m_groupIndex = 0xFF;
            m_saveInDB = true;
