				...
				if (GameMoney.IsItemMoney(lootTemplate.Name))
				{
					...
					if (value != lootTemplate.Price)
					{
						...
						if (killerPlayer != null)
							killerPlayer.Out.SendMessage(LanguageMgr.GetTranslation(killerPlayer.Client, "GameNPC.DropLoot.AdditionalMoney", MoneyUtil.GetString(value - lootTemplate.Price)), EChatType.CT_Loot, EChatLoc.CL_SystemWindow);
					}
					...
					if (killer is GamePlayer)
					{
						...
						if (killerPlayer.GetModified(EProperty.MythicalCoin) > 0)
						{
							...
							killerPlayer.Out.SendMessage(LanguageMgr.GetTranslation(killerPlayer.Client,
																					"GameNPC.DropLoot.ItemAdditionalMoney", MoneyUtil.GetString(value - lootTemplate.Price)), EChatType.CT_Loot, EChatLoc.CL_SystemWindow);
						}
					}
