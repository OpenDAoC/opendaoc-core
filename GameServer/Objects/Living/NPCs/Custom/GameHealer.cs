	...
	public class GameHealer : GameNpc
	{
		...
		public override bool Interact(GamePlayer player)
		{
            ...
            if (player.TotalConstitutionLostAtDeath > 0)
			{
				...
				player.Out.SendCustomDialog(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Dialog.AcceptDecline.Healer", MoneyUtil.GetString(player.TotalConstitutionLostAtDeath * (long)oneConCost)), new CustomDialogResponse(HealerDialogResponse));
            }
		...
		protected void HealerDialogResponse(GamePlayer player, byte response)
        {
            ...
            if (player.RemoveMoney(totalCost))
            {
                ...
                ChatUtil.SendSystemMessage(player, "GameNPC.Interact.YouGiveDonation.Healer", GetPronoun(2, false, player.Client.Account.Language), MoneyUtil.GetString(totalCost));
            ...
            else // If insufficient funds available, throw "error"
            {
	            ...
	            ChatUtil.SendSayMessage(player, "GameNPC.Interact.NeedMoney.Healer", GetName(0, true, player.Client.Account.Language, this), MoneyUtil.GetString(totalCost), restorePoints);
            }
            return;
        }
