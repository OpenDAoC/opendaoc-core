...
{
	[NpcGuild("Enchanter")]
	public class EnchanterNpc : GameNpc
	{
			...
			if (item.Level >= 10 && item.IsCrafted)
			{
				if (item.Object_Type != (int) EObjectType.Magical && item.Object_Type != (int) EObjectType.Bolt && item.Object_Type != (int) EObjectType.Poison)
				{
					if (item.Bonus == 0)
					{
                        ...
                        player.Client.Out.SendCustomDialog(LanguageMgr.GetTranslation(player.Client, "GameNPC.Enchanter.ReceiveItem.Cost", MoneyUtil.GetString(CalculEnchantPrice(item))), new CustomDialogResponse(EnchanterDialogResponse));
                    }
					else
