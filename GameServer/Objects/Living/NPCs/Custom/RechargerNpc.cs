...
public class RechargerNpc : GameNpc
{
    ...
    public override bool ReceiveItem(GameLiving source, InventoryItem item)
    {
        ...
        if (item.Charges < item.MaxCharges)
        {
            ...
            NeededMoney += (item.MaxCharges - item.Charges) * MoneyUtil.GetMoney(0, 0, 10, 0, 0);
        }

        if (item.Charges1 < item.MaxCharges1)
        {
            player.TempProperties.setProperty(RECHARGE_ITEM_WEAK, new WeakRef(item));
            NeededMoney += (item.MaxCharges1 - item.Charges1) * MoneyUtil.GetMoney(0, 0, 10, 0, 0);
        }
        ...
        if (NeededMoney > 0)
        {
            player.Client.Out.SendCustomDialog(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Recharger.ReceiveItem.Cost",
                    MoneyUtil.GetString(NeededMoney)), RechargerDialogResponse);
            return true;
        }
    ...
    private void RechargerDialogResponse(GamePlayer player, byte response)
    {
        ...
        long cost = 0;
        if (item.Charges < item.MaxCharges) cost += (item.MaxCharges - item.Charges) * MoneyUtil.GetMoney(0, 0, 10, 0, 0);

        if (item.Charges1 < item.MaxCharges1)
            cost += (item.MaxCharges1 - item.Charges1) * MoneyUtil.GetMoney(0, 0, 10, 0, 0);
        ...
        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                ...
                GetName(0, false, player.Client.Account.Language, this), MoneyUtil.GetString(cost)),
    ...
    private void AskRechargeAll(GamePlayer player)
    {
        ...
        if (TotalCost > 0)
            player.Client.Out.SendCustomDialog(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Recharger.ReceiveItem.Cost",
                MoneyUtil.GetString(TotalCost)), RechargeAll);
        else
            SayTo(player, EChatLoc.CL_PopupWindow,
