...
namespace DOL.GS
{
    public class BossGiantSporiteCluster : GameEpicBoss
    {
        private static new readonly log4net.ILog log =
            log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public BossGiantSporiteCluster()
            : base()
        {
        }
    ...
    public class GSCAdds : GameEpicNpc
    {
        ...
        public override void TakeDamage(GameObject source, EDamageType damageType, int damageAmount, int criticalAmount)
        {
            ...
            foreach (GameNpc copy in GetNPCsInRadius(10000))
            {
                if (copy != null)
                {
                    if ((copy is GSCAdds || copy is BossGiantSporiteCluster) && this != copy && copy.IsAlive)
                    {
                        copy.Health = Health;
                    }
                }
        ...
        public override void Die(GameObject killer)
        {
            ...
            foreach (GameNpc boss in GetNPCsInRadius(10000))
            {
                if (boss != null)
                {
                    if (this != boss && boss is BossGiantSporiteCluster && boss.IsAlive)
                    {
                        boss.Die(boss);
                    }
                }
