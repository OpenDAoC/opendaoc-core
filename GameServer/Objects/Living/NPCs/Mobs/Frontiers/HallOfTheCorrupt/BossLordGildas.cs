...
namespace DOL.GS
{
    public class BossLordGildas : GameEpicBoss
    {
        public BossLordGildas() : base()
        {
        }
        ...
        public static void ScriptLoaded(CoreEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                BossLordGildas HOC = new BossLordGildas();
        ...
        {       
            ...
            else if (Body.InCombatInLast(30 * 1000) == false && this.Body.InCombatInLast(35 * 1000))
            {
                ...
                if (Reset_Gildas == false)
                {
                    ...
                    if (!Body.Styles.Contains(BossLordGildas.slam))
                        Body.Styles.Add(BossLordGildas.slam);
                    if (!Body.Styles.Contains(BossLordGildas.taunt))
                        Body.Styles.Add(BossLordGildas.taunt);
                    if (!Body.Styles.Contains(BossLordGildas.BackStyle))
                        Body.Styles.Add(BossLordGildas.BackStyle);
                    if (!Body.Styles.Contains(BossLordGildas.Taunt2h))
                        Body.Styles.Remove(BossLordGildas.Taunt2h);
                    Body.Strength = npcTemplate.Strength;
                    Body.ParryChance = npcTemplate.ParryChance;
                    Body.BlockChance = npcTemplate.BlockChance;
                    Stage2 = false;
                    Body.styleComponent.NextCombatStyle = BossLordGildas.taunt;
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (Body.TargetObject != null)
                {
                    if (Stage2 == false)
                    {
                        ...
                        if (angle >= 160 && angle <= 200)
                        {
                            ...
                            Body.styleComponent.NextCombatBackupStyle = BossLordGildas.Taunt2h;
                            Body.styleComponent.NextCombatStyle = BossLordGildas.BackStyle;//do backstyle when angle allow it
                        }
                        else
                        {
                            Body.Strength = npcTemplate.Strength;
                            ...
                            Body.SwitchWeapon(EActiveWeaponSlot.Standard);
                            Body.VisibleActiveWeaponSlots = 16;
                            Body.styleComponent.NextCombatStyle = BossLordGildas.taunt;//if not backstyle for angle then do taunt
                        }
                        if (!target.effectListComponent.ContainsEffectForEffectType(EEffect.Stun) && !target.effectListComponent.ContainsEffectForEffectType(EEffect.StunImmunity))
                        {
                            ...
                            Body.ParryChance = 25;
                            Body.BlockChance = 80;
                            Body.styleComponent.NextCombatStyle = BossLordGildas.slam;//check if target has stun or immunity if not slam
                        }
                ...
                if(Body.HealthPercent < 50 && Stage2==false)//boss change to polearm armsman
                {
                    ...
                    Body.Inventory = template.CloseTemplate();
                    if(Body.Styles.Contains(BossLordGildas.slam))
                        Body.Styles.Remove(BossLordGildas.slam);
                    if (Body.Styles.Contains(BossLordGildas.taunt))
                        Body.Styles.Remove(BossLordGildas.taunt);
                    if (Body.Styles.Contains(BossLordGildas.BackStyle))
                        Body.Styles.Remove(BossLordGildas.BackStyle);
                    if (Body.Styles.Contains(BossLordGildas.Taunt2h))
                        Body.Styles.Remove(BossLordGildas.Taunt2h);
                    Stage2 = true;
                }
                if(Stage2 == true)
                {
                    Body.styleComponent.NextCombatBackupStyle = BossLordGildas.PoleAnytimer;
                    Body.styleComponent.NextCombatStyle = BossLordGildas.AfterStyle;
                    Body.Strength = 340;
                    Body.SwitchWeapon(EActiveWeaponSlot.TwoHanded);
