...
namespace DOL.GS
{
    public class ApocInitializator : GameNpc
    {
        ...
        public void StartRespawnEncounter()
        {
            if (IsAlive)
            {
                if (!BossFames.FamesIsUp && !BossBellum.BellumUP && !BossMorbus.MorbusUP && !BossFunus.FunusUp && !BossApocalypse.ApocUP && !start_respawn_check)
                {
...
namespace DOL.GS
{
    public class BossFames : GameEpicBoss
    {
        public BossFames() : base() { }
...
namespace DOL.GS
{
    public class BossBellum : GameEpicBoss
    {
        public BossBellum() : base() { }
        public override bool HasAbility(string keyName)
        {
            if (IsAlive && keyName == GS.Abilities.CCImmunity)
                return true;
...
namespace DOL.GS
{
    public class BossMorbus : GameEpicBoss
    {
        public BossMorbus() : base() { }
        public override bool HasAbility(string keyName)
        {
            if (IsAlive && keyName == GS.Abilities.CCImmunity)
                return true;
        ...
        {
            BossFunus Add = new BossFunus();//inside controller
            Add.X = 29467;
            Add.Y = 25235;
            Add.Z = 19490;
            Add.CurrentRegion = this.CurrentRegion;
...
namespace DOL.AI.Brain
{
    public class MorbusBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
                    ...
                    foreach (GameNpc npc in Body.GetNPCsInRadius(4000))
                    {
                        if (npc != null)
                        {
                            if (npc.IsAlive)
                            {
                                ...
                                {
                                    npc.RemoveFromWorld();
                                    BossMorbus.Morbus_Swarm_count = 0;
                                }
                            }
                        }
                    }
            ...
            {
                StartedMorbus = true;
                if(BossMorbus.Morbus_Swarm_count > 0)
                {
                    Body.Model = 771;
                    Body.Size = 50;
                    IsBug = true;                   
...
namespace DOL.GS
{
    public class BossFunus : GameEpicBoss
    {
        public BossFunus() : base() { }
        ...
        {
            BossApocalypse Add = new BossApocalypse();
            Add.X = 29467;
            Add.Y = 25235;
            Add.Z = 19490;
            Add.CurrentRegion = this.CurrentRegion;
...
namespace DOL.GS
{
    public class BossApocalypse : GameEpicBoss
    {
        public BossApocalypse() : base() { }
        public void BroadcastMessage(String message)
        {
        ...
        public override bool AddToWorld()
        {
            ...
            ApocalypseBrain.StartedApoc = false;
            EpicHarbringerOfFate.HarbringersCount = 0;
            ApocUP = true;
    ...
    public class ApocalypseBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive)//bring mobs from rooms if mobs got set PackageID="ApocBaf"
            {
                ...
                if (EpicHarbringerOfFate.HarbringersCount == 0)
                {
        ...
        {
            if (BossApocalypse.KilledEnemys == 4)//he doint it only once, spawning 2 harbringers is killed 4 players
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
                {
                    if (player != null)
                ...
                for (int i = 0; i < 2; i++)
                {
                    EpicHarbringerOfFate Add = new EpicHarbringerOfFate();
                    ...
                    Add.AddToWorld();
                    ++EpicHarbringerOfFate.HarbringersCount;
                }
            }
        ...
        {
            EpicRainOfFire Add = new EpicRainOfFire();
            Add.X = Body.X;
            Add.Y = Body.Y;
            Add.Z = Body.Z + 940;
            Add.CurrentRegionID = Body.CurrentRegionID;
...
namespace DOL.GS
{
    public class EpicHarbringerOfFate : GameEpicNpc
    {
        public EpicHarbringerOfFate() : base() { }
        public override double AttackDamage(InventoryItem weapon)
        {
...
namespace DOL.GS
{
    public class EpicRainOfFire : GameEpicNpc
    {
        public EpicRainOfFire() : base() { }
        public override double AttackDamage(InventoryItem weapon)
        {
            return base.AttackDamage(weapon) * Strength / 100;
        }
