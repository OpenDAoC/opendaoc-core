    ...
    public class HostInitializator : GameNpc
    {
        ...
        public void DoStuff()
        {
            if (this.IsAlive)
            {
                ...
                if (Spawnhost == false && BossHost.HostCount == 0)
                {
                    SpawnHostCopy();
                    Spawnhost = true;
                }

                if (pickhostcheck == false && BossHost.HostCount > 0)
                {
                    PickHost();
                    pickhostcheck = true;
                }

                if (BossHost.HostCount == 0 && DoRespawnTimer == false)
                {
                    RespawnChecker();
                    DoRespawnTimer = true;
                }
        ...
        public void SpawnHostCopy()
        {
            ...
            for (BossHost.HostCount = 0; BossHost.HostCount < 8; BossHost.HostCount++)
            {
                BossHost Add = new BossHost();
                Add.X = this.X;
                Add.Y = this.Y;
                Add.Z = this.Z;
                ...
                Add.AddToWorld();
                Add.OrbsReward = 10;
                Add.PackageID = "HostCopy" + BossHost.HostCount;
            }
        }
...
namespace DOL.GS
{
    public class BossHost : GameEpicBoss
    {
        public BossHost() : base()
        {
        }
