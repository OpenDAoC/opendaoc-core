...
namespace DOL.GS
{
	...
	public class GameMerchant : GameNpc
	{
		...
		public virtual void OnPlayerSell(GamePlayer player, InventoryItem item)
		{
			...
			{
				string message = LanguageMgr.GetTranslation(player.Client.Account.Language, "GameMerchant.OnPlayerSell.GivesYou", GetName(0, true), MoneyUtil.GetString(itemValue), item.GetName(0, false));
		...
		public virtual long OnPlayerAppraise(GamePlayer player, InventoryItem item, bool silent)
		{
			...
			if (!silent)
			{
				...
				{
					message = LanguageMgr.GetTranslation(player.Client.Account.Language, "GameMerchant.OnPlayerAppraise.Offers", GetName(0, true), MoneyUtil.GetString(val), item.GetName(0, false));
				}
