	...
	public class GameGuardMerchant : GameKeepGuard
	{
			...
			lock (player.Inventory)
			{
				...
				if (player.GetCurrentMoney() < totalValue)
				{
					player.Out.SendMessage(
						LanguageMgr.GetTranslation(player.Client.Account.Language, "GameMerchant.OnPlayerBuy.YouNeed",
							MoneyUtil.GetString(totalValue)), EChatType.CT_System, EChatLoc.CL_SystemWindow);
					return;
				}
				...
				if (amountToBuy > 1)
					message = LanguageMgr.GetTranslation(player.Client.Account.Language,
						...
						MoneyUtil.GetString(totalValue));
				else
					message = LanguageMgr.GetTranslation(player.Client.Account.Language,
						"GameMerchant.OnPlayerBuy.Bought", template.GetName(1, false), MoneyUtil.GetString(totalValue));
		...
		public virtual void OnPlayerSell(GamePlayer player, InventoryItem item)
		{
			...
			if (player.Inventory.RemoveItem(item))
			{
				string message = LanguageMgr.GetTranslation(player.Client.Account.Language,
					"GameMerchant.OnPlayerSell.GivesYou", GetName(0, true), MoneyUtil.GetString(itemValue),
		...
		public virtual long OnPlayerAppraise(GamePlayer player, InventoryItem item, bool silent)
		{
			...
			if (!silent)
			{
				...
				{
					message = LanguageMgr.GetTranslation(player.Client.Account.Language,
						"GameMerchant.OnPlayerAppraise.Offers", GetName(0, true), MoneyUtil.GetString(val),
						item.GetName(0, false));
				}
