...
namespace DOL.GS.Keeps
{
	...
	/// </summary>
	public class GuardInventoryMgr
	{
		...
		public static void EquipGuard(GameKeepGuard guard)
		{
            ...
            if (guard is FrontierHastener || guard is GateKeeper || guard is GateKeeperOut)
            {
                switch (guard.Realm)
                {
                    ...
                    case ERealm.Midgard:
                        {
                            guard.Inventory = GuardInventoryMgr.Midgard_Hastener.CloneTemplate();
                            break;
                        }
                }
            }
				...
				case ERealm.Albion:
					{
						if (guard is GuardFighter)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Albion_FighterPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Albion_Fighter.CloneTemplate();
						}
						else if (guard is GuardCommander)
							guard.Inventory = GuardInventoryMgr.Albion_Commander.CloneTemplate();
						else if (guard is GuardRelicKeepFighter)
                            guard.Inventory = GuardInventoryMgr.Albion_FighterPK.CloneTemplate();
						else if (guard is GuardLord || guard is MissionMaster)
							guard.Inventory = GuardInventoryMgr.Albion_Lord.CloneTemplate();
						else if (guard is RvrCraftingMerchant)
							guard.Inventory = GuardInventoryMgr.Albion_Merchant.CloneTemplate();
						else if (guard is GuardCurrencyMerchant)
							guard.Inventory = GuardInventoryMgr.Albion_Merchant.CloneTemplate();
						else if (guard is GuardHealer)
							guard.Inventory = GuardInventoryMgr.Albion_Healer.CloneTemplate();
						else if (guard is GuardArcher)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Albion_ArcherPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Albion_Archer.CloneTemplate();
						}
						else if (guard is GuardCaster)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Albion_CasterPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Albion_Caster.CloneTemplate();
						}
						else if (guard is GuardStealther)
							guard.Inventory = GuardInventoryMgr.Albion_Stealther.CloneTemplate();
						break;
					}
				case ERealm.Midgard:
					{
						...
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Midgard_FighterPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Midgard_Fighter.CloneTemplate();
						}
						else if (guard is GuardCommander)
							guard.Inventory = GuardInventoryMgr.Midgard_Commander.CloneTemplate();
                        else if (guard is GuardRelicKeepFighter)
                            guard.Inventory = GuardInventoryMgr.Midgard_FighterPK.CloneTemplate();
                        else if (guard is GuardLord|| guard is MissionMaster)
							guard.Inventory = GuardInventoryMgr.Midgard_Lord.CloneTemplate();
						else if (guard is RvrCraftingMerchant)
							guard.Inventory = GuardInventoryMgr.Midgard_Merchant.CloneTemplate();
						else if (guard is GuardCurrencyMerchant)
							guard.Inventory = GuardInventoryMgr.Midgard_Merchant.CloneTemplate();
						else if (guard is GuardHealer)
							guard.Inventory = GuardInventoryMgr.Midgard_Healer.CloneTemplate();
						else if (guard is GuardArcher)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Midgard_ArcherPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Midgard_Archer.CloneTemplate();
						}
						else if (guard is GuardCaster)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Midgard_CasterPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Midgard_Caster.CloneTemplate();
						}
						else if (guard is GuardStealther)
							guard.Inventory = GuardInventoryMgr.Midgard_Stealther.CloneTemplate();
						break;
					}
				case ERealm.Hibernia:
					{
						...
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Hibernia_FighterPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Hibernia_Fighter.CloneTemplate();
						}
						else if (guard is GuardCommander)
							guard.Inventory = GuardInventoryMgr.Hibernia_Commander.CloneTemplate();
                        else if (guard is GuardRelicKeepFighter)
                            guard.Inventory = GuardInventoryMgr.Hibernia_FighterPK.CloneTemplate();
                        else if (guard is GuardLord || guard is MissionMaster)
							guard.Inventory = GuardInventoryMgr.Hibernia_Lord.CloneTemplate();
						else if (guard is RvrCraftingMerchant)
							guard.Inventory = GuardInventoryMgr.Hibernia_Merchant.CloneTemplate();
						else if (guard is GuardCurrencyMerchant)
							guard.Inventory = GuardInventoryMgr.Hibernia_Merchant.CloneTemplate();
						else if (guard is GuardHealer)
							guard.Inventory = GuardInventoryMgr.Hibernia_Healer.CloneTemplate();
						else if (guard is GuardArcher)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Hibernia_ArcherPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Hibernia_Archer.CloneTemplate();
						}
						else if (guard is GuardCaster)
						{
							if (guard.IsPortalKeepGuard || guard.Level == 255)
								guard.Inventory = GuardInventoryMgr.Hibernia_CasterPK.CloneTemplate();
							else guard.Inventory = GuardInventoryMgr.Hibernia_Caster.CloneTemplate();
						}
						else if (guard is GuardStealther)
							guard.Inventory = GuardInventoryMgr.Hibernia_Stealther.CloneTemplate();
						break;
					}
			}
			...
			if (item != null)
			{
				...
				{
					item.Extension = (int)EExtensionLevel.Five;
				}
				else
				{
					item.Extension = (int)EExtensionLevel.Four;
            ...
            else if ((guard is GuardFighter || guard is GuardCommander || guard is GuardLord || guard is GuardRelicKeepFighter) && UtilCollection.Chance(50))
				guard.SwitchWeapon(EActiveWeaponSlot.TwoHanded);
