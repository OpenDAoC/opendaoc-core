...
namespace DOL.GS.Keeps
{
    ...
    /// </summary>
    public class GateKeeper : GameKeepGuard
    {
        private static new readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        private List<DbKeepDoorTeleports> m_destinationsIn = new List<DbKeepDoorTeleports>();
        ...
        protected void LoadDestinations()
        {
            ...
            this.m_destinationsIn.AddRange(GameServer.Database.SelectObjects<DbKeepDoorTeleports>(DB.Column("Text").IsEqualTo("sortir").And(DB.Column("TeleportType").IsEqualTo(type))));
        }
        ...
        protected virtual void OnTeleportSpell(GamePlayer player, DbKeepDoorTeleports destination, bool delayed)
        {
            if (destination == null)
                return;
        ...
        {
            DbKeepDoorTeleports port = null;
            if (text.ToLower().Contains("leave") || text.ToLower().Contains("sortir") || text == "interact")
            {

                foreach (DbKeepDoorTeleports t in m_destinationsIn)
                {
                    if (t == null) break;
    ...
    {
        private static new readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        private List<DbKeepDoorTeleports> m_destinationsOut = new List<DbKeepDoorTeleports>();
        ...
        protected void LoadDestinations()
        {
            ...
            this.m_destinationsOut.AddRange(GameServer.Database.SelectObjects<DbKeepDoorTeleports>(DB.Column("Text").IsEqualTo("entrer").And(DB.Column("TeleportType").IsEqualTo(type))));
        }
        ...
        protected virtual bool GetTeleportLocation(GamePlayer player, string text)
        {
            ...
            {
                foreach (DbKeepDoorTeleports t in this.m_destinationsOut)
                {
                    if (t == null) break;
                    if (t.KeepID == Component.Keep.KeepID)
                    {
